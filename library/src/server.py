...
def borrow_book():
    ...
    if not book_key:
        return jsonify({"status": "error", "message": "Thiếu book_key"}), 400
    ...
    if existing:
        conn.close()
        return jsonify({"status": "exists", "message": "Bạn đã mượn sách này trước đó"}), 200
    ...
    return jsonify({"status": "success", "message": "Mượn sách thành công"}), 201
...
def return_book(book_key):
    ...
    if deleted == 0:
        return jsonify({"status": "error", "message": "Không tìm thấy sách để trả"}), 404
    return jsonify({"status": "success", "message": "Đã trả sách"}), 200
